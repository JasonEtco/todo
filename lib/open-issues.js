...
module.exports = async (context, robot) => {
  ...
  const headSha = await context.github.repos.getShaOfCommitRef(context.repo({ref: `refs/heads/${context.payload.repository.default_branch}`}))
  if (context.payload.head_commit.id === headSha.data.sha) return
