...
module.exports = function generateBody (context, config, title, file, contents, author, sha, pr) {
  const separator = '\n\n---\n\n'
  const footer = `${separator}###### This issue was generated by [todo](https://github.com/jasonetco/todo) based on a \`${config.keyword}\` comment in ${sha}${pr ? ` in #${pr}` : ''}. It's been assigned to @${author} because they committed the code.`

  const re = new RegExp(`${title}\n.*@body (.*)`, 'gim')
  const bodyRegex = re.exec(contents)
  const body = bodyRegex ? bodyRegex[1] + separator : ''

  const showBlob = !Number.isNaN(config.blobLines) && config.blobLines !== 0
  const blob = showBlob ? generateBlobLink(context, file, contents, title, sha, config) : ''
