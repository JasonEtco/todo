...
async function search (robot, context, cfg, opts) {
  if (Array.isArray(cfg.keyword)) {
    for (let i = 0; i < cfg.keyword.length; i++) {
      const keyword = cfg.keyword[i]
      await search(robot, context, { ...cfg, keyword }, opts)
    }
    return
  }

  const { contents, title, file, pull_request, sha, number, labels } = opts

  // Check if contents contains the TODO title
  const matches = searchContentsForTitles(contents, cfg, title)
  if (!matches) return
