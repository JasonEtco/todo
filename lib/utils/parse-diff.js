...
module.exports = ({ match, context, config }) => {
  ...
  const totalLines = addedLines.split(lineSplit).length

  let range
  if (start === totalLines) {
    // The matched line is at the end of the file
    range = `L${start}`
  } else if (config.blobLines && start + config.blobLines > totalLines) {
    // The file has fewer lines than possible with start + blobLines
    range = `L${start}-L${totalLines - config.blobLines}`
