...
const addAt = str => !str.startsWith('@') ? `@${str}` : str

function generateAssignedTo (autoAssign, author = 'JasonEtco') {
  if (autoAssign === true) {
...
function generateFooter (cfg, keyword) {
  const Footer = (props) => [<hr key={1} />, <h6 key={2}>This issue was generated by <a href={c.app}>todo</a> based on a <code>{keyword}</code> comment in <a href="#"><code>{c.sha}</code></a>. {props.children}</h6>]
  return <Footer>{generateAssignedTo(cfg.autoAssign)}</Footer>
}

export default class Issue extends Component {
  render () {
    const { title, body, start, code, cfg, keyword } = this.props
